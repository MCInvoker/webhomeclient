...
function DragToUnlock ({ onSuccess, sliderContainerStyle = {} }) {
    ...
    const handleMouseMove = (e) => {
        if (isDragging) {
            const newPosition = Math.min(
                ...
                sliderRef.current.offsetWidth - 32,
            );
            setPosition(newPosition);
            if (newPosition >= sliderRef.current.offsetWidth - 32) {
                // 40是滑块的宽度
                setIsDragging(false);
                onSuccess(); // 验证成功的回调
                setIsSuccess(true);
    ...
    return (
        ...
        >
            <div
                ...
                ref={sliderRef}
            >
                <div className="slider-tip">{isSuccess ? '验证通过' : '请将滑块拖动到最右边'}</div>
